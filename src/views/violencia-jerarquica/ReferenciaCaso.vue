...
const save = async () => {
    ...
    const payload = {
        ...
        id_violencia_victima: caso.value.id_victima,
    ...
    const save = await ViolenciaJerarquica.createReferenciaCaso(payload).then((res) => {
        if(res.status === 201){
...
const validateForm = () => {
    ...
    if (!form.value.referenciaDenuncia) validationErrors.value['referenciaDenuncia'] = true;
    else delete validationErrors.value['referenciaDenuncia'];

    if (!form.value.fechaReferenciaDenuncia) validationErrors.value['fechaReferenciaDenuncia'] = true;
    else delete validationErrors.value['fechaReferenciaDenuncia'];

    if (!form.value.instanciaReferencia) validationErrors.value['instanciaReferencia'] = true;
    else delete validationErrors.value['instanciaReferencia'];
