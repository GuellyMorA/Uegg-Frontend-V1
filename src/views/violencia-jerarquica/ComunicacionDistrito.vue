...
const save = async () => {
    ...
    const payload = {
        ...
        fec_comunicacion_distrital: datePartsDist[2] || '' +'-'+ datePartsDist[1] || '' +'-'+ datePartsDist[0] || '',
    ...
    const save = await ViolenciaJerarquica.createComunicacionDistrito(payload).then((res) => {
        if(res.status === 201){
