...
const save = async () => {
    ...
    const payload = {    
        ...
        ini_victi: form.value.inicialVictima,
        id_violencia_agresor_nombre: caso.value.id_agresor,
        ini_agre: form.value.inicialAgresor,
        remision_dir_deptal: form.value.remisionCasoDepartamentoMedidas,
        fec_agresion: dateParts[2] || '' +'-'+ dateParts[1] || '' +'-'+ dateParts[0] || '',
        id_violencia_sanciones_tipo: form.value.sancion.id,
        cumplir_sancion: form.value.sancionCumplimiento,
        comu_tutor: form.value.sancionAvisoProgenitor,
    ...
    const save = await ViolenciaJerarquica.createSeguimientoSancion(payload).then((res) => {
        if(res.status === 201){
            ...
            <v-card elevation="10" class="withbg">
                <v-card-item>
                    <div class="d-sm-flex align-center justify-space-between pt-sm-2">
                        <v-card-title class="text-h5">Seguimiento al caso - sanciones por falta graves </v-card-title>
                    </div>
